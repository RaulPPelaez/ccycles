...
static int send_color_packet(int sock, Rgb color) {
  uint32_t payload_len_be = htonl(3u);
  unsigned char buf[7];
...
static int send_game_state_packet(GameServer *s, int sock) {
  ...
  packet_size += 4 * 3; // gridWidth, gridHeight, playerCount
  for (uint32_t i = 0; i < player_count; ++i) {
    size_t name_len = strlen(player_ptrs[i]->name);
    packet_size += 4 * 2;        // x, y
    packet_size += 3;            // r,g,b
  ...
  packet_size += (size_t)w * (size_t)h; // grid bytes

  uint32_t payload_len_be = htonl((uint32_t)packet_size);
  ...
  for (uint32_t i = 0; i < player_count; ++i) {
    ...
    int32_t xi = p->position.x, yi = p->position.y;
    uint32_t x_be = htonl((uint32_t)xi);
    uint32_t y_be = htonl((uint32_t)yi);
    if (send_all(sock, &x_be, 4) < 0 || send_all(sock, &y_be, 4) < 0)
      return -1;
    unsigned char rgb[3] = {p->color.r, p->color.g, p->color.b};
    if (send_all(sock, rgb, 3) < 0)
      return -1;
    uint32_t name_len = (uint32_t)strlen(p->name);
    uint32_t name_len_be = htonl(name_len);
    if (send_all(sock, &name_len_be, 4) < 0)
      return -1;
    if (name_len && send_all(sock, p->name, name_len) < 0)
      return -1;
    uint8_t idb = p->id;
    if (send_all(sock, &idb, 1) < 0)
      return -1;
  ...
  // Grid
  size_t grid_sz = (size_t)w * (size_t)h;
...
void server_run(GameServer *s) {
  ...
  while (s->running && !game_is_over(s->game)) {
    ...
    // Clone current active clients
    bool clients_unsent[256] = {false};
    bool to_recv[256] = {false};
    Direction directions[256];
    ...
    for (;;) {
      ...
      for (int id = 1; id < 256; ++id) {
        if (clients_unsent[id]) {
          if (send_game_state_packet(s, s->client_sockets[id]) == 0) {
      ...
      int waiting = 0;
      for (int id = 1; id < 256; ++id) {
        if (to_recv[id] && s->client_sockets[id] >= 0) {
          FD_SET(s->client_sockets[id], &rfds);
          if (s->client_sockets[id] > maxfd)
            maxfd = s->client_sockets[id];
          waiting++;
        }
      }
      if (waiting == 0) {
        ...
        for (int id = 1; id < 256; ++id) {
          ...
          if (to_recv[id] && sock >= 0 && FD_ISSET(sock, &rfds)) {
            ...
            if (recv_move_direction(sock, &dir) == 0) {
              ulog_trace("server_run: received direction %d from client %d",
                         dir, id);
              if (dir < 0)
                dir = 0;
              if (dir > 3)
              ...
              ulog_warn("server_run: failed to recv from client %d, dropping",
                        id);
      ...
      long elapsed_ms = (now.tv_sec - comm_start.tv_sec) * 1000L +
                        (now.tv_usec - comm_start.tv_usec) / 1000L;
      ...
      for (int id = 1; id < 256; ++id)
        if (clients_unsent[id] || to_recv[id])
          remaining++;
      if (remaining == 0) {
        ulog_trace("server_run: all clients processed, breaking comm loop");
        break;
      }
    }

    // Move players with whatever inputs we got
