...
typedef struct {
  ...
  uint8_t *grid;         ///< Grid cells (size = grid_width * grid_height)
...
static int recv_sfml_packet_len(int fd, uint32_t *out_len) {
  uint32_t be = 0;
...
int recv_sfml_color(int fd, Rgb *out) {
  if (!out) {
    errno = EINVAL;
    return -1;
  }
...
static int recv_sfml_packet(int fd, uint8_t **out, uint32_t *out_len) {
  uint32_t be = 0;
  if (recv_all(fd, &be, sizeof(be)) < 0)
    return -1;
 ...
 */
static void free_game_state(GameState *gs) {
  if (!gs)
    return;
  if (gs->players) {
 ...
 * @return 0 on success, -1 on failure (check errno)
 */
int recv_game_state(SOCKET sock, GameState *out) {
  if (!out) {
    errno = EINVAL;
    return -1;
  }
  ...
  out->players = (Player *)calloc(out->player_count, sizeof(Player));
  if (!out->players) {
    free(pkt);
    free_game_state(out);
    return -1;
  ...
  if (grid_sz) {
    out->grid = (uint8_t *)malloc(grid_sz * sizeof(uint8_t));
    if (!out->grid) {
      free(pkt);
      free_game_state(out);
      return -1;
